...
import { isHostedPlatform } from './platform';
...
// On Render/Railway, use dashboard variables only — never override with a local file.
if (!isHostedPlatform()) {
...
export const loadedEnvFile =
  !isHostedPlatform() && fs.existsSync(envFile)
    ? `.env.${nodeEnv}`
    : !isHostedPlatform() && nodeEnv !== 'production' && fs.existsSync(legacyEnvFile)
      ? '.env'
      : null;
